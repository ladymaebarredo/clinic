...
export function ItemModal({ onClose, item, revalidate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = item
        ? await updateItem(item.id, itemName, itemType, quantity, expiryDate) // Update existing item
        : await createItem(itemName, itemType, quantity, expiryDate); // Create new item
